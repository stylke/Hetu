    ...
    for ep in range(num_epochs):
        ...
        for train_file in train_files:
            ...
            for i in range(dataloader.batch_num):
                ...
                attention_mask = torch.LongTensor(batch_data['attention_mask']).to(device).to(torch.bfloat16)
                ...
                opt.zero_grad()
                _,_, masked_lm_loss_mean, next_sentence_loss_mean, tmp1, tmp2 = model(input_ids, token_type_ids, attention_mask, masked_lm_labels, next_sentence_label)   
                ...
                print("lala:", masked_lm_loss_mean)
                next_sentence_loss_out = next_sentence_loss_mean.item()
                loss_out = loss.item()

                end_time = time.time()
                # print(tmp1, tmp2)
                print('[Epoch %d] (Iteration %d/%d): Loss = %.3f, MLM_loss = %.3f, NSP_loss = %.6f, Time = %.3f'%(ep,step_num,dataloader.batch_num,loss_out, masked_lm_loss_out, next_sentence_loss_out, end_time-start_time))
                step_num += 1
                global_step_num += 1
