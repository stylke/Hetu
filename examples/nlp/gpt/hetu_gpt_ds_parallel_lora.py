...
class GPTBlock(ht.nn.Module):
    def __init__(self, config, ds_parallel_config, layer_idx):
        ...
        self.ln_1 = ht.nn.HtParallelLayerNorm(hidden_size, ds_parallel_config['layernorm1'], 
                                              eps=config.layer_norm_epsilon, 
                                              dtype = config.lora_dtype, name=f'ln1_block{layer_idx}')
        self.attn = GPTAttention(config, ds_parallel_config['attn'], layer_idx=layer_idx, name=f'attn_block{layer_idx}')
        self.ln_2 = ht.nn.HtParallelLayerNorm(hidden_size, ds_parallel_config['layernorm2'],
                                              eps=config.layer_norm_epsilon, 
                                              dtype = config.lora_dtype, name=f'ln2_block{layer_idx}')
...
class GPTModel(ht.nn.Module):
    def __init__(self, config, ds_parallel_config):
        ...
        self.embed_dim = config.hidden_size
        self.wte = ht.nn.HtVocabParallelEmbedding(config.vocab_size, self.embed_dim, 
                                                  ds_parallel_config['wte'], 
                                                  dtype = config.lora_dtype, name='wte')
        # self.wpe = ht.nn.HtVocabParallelEmbedding(config.max_position_embeddings, self.embed_dim, 
        #                                      ds_parallel_config['wte'], 
        #                                      dtype = config.lora_dtype, name='wpe')
        self.wpe = ht.nn.HtParallelEmbedding(config.max_position_embeddings, self.embed_dim, 
                                             ds_parallel_config['wpe'], 
                                             dtype = config.lora_dtype, name='wpe')
        ...
        self.ln_f = ht.nn.HtParallelLayerNorm(self.embed_dim, ds_parallel_config['layernorm_final'], 
                                              eps=config.layer_norm_epsilon, 
                                              dtype = config.lora_dtype, name='ln_final')
    ...
    def forward(
        ...
        input_ids = input_ids.reshape([mbs_times_dp * seq_len])
        position_ids = position_ids.reshape([mbs_times_dp * seq_len])
        inputs_embeds = self.wte(input_ids) # [b, seq_len, embed_dim]
        position_embeds = self.wpe(position_ids) # [b, seq_len, embed_dim]
        inputs_embeds = inputs_embeds.reshape([mbs_times_dp, seq_len, -1])
        position_embeds = position_embeds.reshape([mbs_times_dp, seq_len, -1])
        ...
        if token_type_ids is not None:
            token_type_ids = token_type_ids.reshape([mbs_times_dp * seq_len])
            token_type_embeds = self.wte(token_type_ids) # [b, seq_len, embed_dim]
            token_type_embeds = token_type_embeds.reshape([mbs_times_dp, seq_len, -1])
