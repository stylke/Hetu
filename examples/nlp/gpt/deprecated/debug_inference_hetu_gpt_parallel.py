...
# todo: 目前只支持贪心sample
def sample(lm_logits):
    
    lm_logits_data = lm_logits.numpy(force=True) # [batch_size, seq_len, vocab_size]
    print('lm_logits:', lm_logits_data[:, -1, :])
    token_ids_data = np.argmax(lm_logits_data[:, -1, :], axis=-1).reshape(-1, 1) # [batch_size, 1]
    return token_ids_data


# 第一次运行，从prefix得到kv cache
# 从第二次运行开始，每次得到一个新的token
def run_graph(feed_dict, debug_info=[]):
    
    # print("FeedDict:", feed_dict)
    results = debug_info[0].graph.run(debug_info[0], debug_info, feed_dict = feed_dict)
    
    for i in range(len(debug_info)):
        print(f'device = {local_device}, debug_info[{i}]:', results[i].numpy(force=True).shape, results[i].numpy(force=True))
...
def inference(args):
    ...
    max_len = 5 # dynamic seq_len will pad to max_len
    ...
    debug_info = model(input_ids=prefix_ids, # dynamic runtime shape [batch_size, seq_len], padding to [batch_size, max_len]
    ...
    new_debug_info = model(input_ids=token_ids, # [batch_size, 1]
    ...
    # todo: 目前只支持batch内等长的inference，不支持dynamic batch
    input = ['Hello, I am a',
    ...
    for i in range(len(input) // batch_size):
        ...
        basic_len = 5
        cur_len = basic_len
        # assume initial dynamic shape is [batch_size, 10]
        ...
        # run graph
        run_graph(kv_cache_feed_dict, debug_info=[debug_info])
        return
        ...
        while cur_len < max_len:
            ...
            # run graph
            kv_cache_tuple, new_token_ids_numpy = run_graph(generation_feed_dict, new_key_values, new_lm_logits)
