...
class Loss(ht.nn.Module):
    ...
    def forward(
        ...
        if labels is not None:
            ...
            loss_unreduced = ht.vocab_parallel_cross_entropy(lm_logits, labels, ignored_index = -1, reduction = "none")
            loss_unreduced = ht.div(loss_unreduced, per_gbs_tokens)
            print(f"loss_unreduced:{loss_unreduced.distributed_states}")
            loss = ht.sum(loss_unreduced)
        return loss
