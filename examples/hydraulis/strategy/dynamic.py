...
def dynamic_strategy(strategy_pool, strategy_id_list: List[int], max_seqlen_list: List[int], cur_strategy_id: int, batch_seqlen_array: np.ndarray):
    ...
    # 线性规划输入数据
    assert batch_seqlen_array.shape().size() == 1, "sorted_len shape must be [global_batch_size,]"
    ...
    # 定义最终优化的目标f
    def f(strategy_id, max_seqlen, select_list, seqlen_list):
        time_sum = 0
    ...
    for i in range(B):
        prob += pulp.lpSum(m[i][j] for j in range(J[i])) == 1
    # 添加约束条件
    ...
    # minimize Z and Z >= f(...)
    for j in range(DP):
        prob += Z >= f(j, Y[j], [m[i][j] for i in range(B)], S)
    # 设置目标函数
    prob += Z
    # 求解问题
    prob.solve()
