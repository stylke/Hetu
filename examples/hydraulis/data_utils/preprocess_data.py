...
from data_utils.tokenizer.tokenizer import build_tokenizer
...
def get_args():
    ...
    group.add_argument('--json-keys', nargs='+', default=['text'],
                       help='space separate listed of keys to extract from json')  # json文件中哪些key存放的内容作为数据
    group.add_argument('--split-sentences', action='store_true',
                       help='Split documents into sentences.')  #按照句子拆分文档
    group.add_argument('--keep-newlines', action='store_true',
                       help='Keep newlines between sentences when splitting.')   # 保留换行符
    ...
    group.add_argument('--keep-sequential-samples', action='store_true',
                       help='Ensure ordering of samples in .jsonl files is '
                            'preserved when using partitions>1.')  #保证分区后的数据是连续的数据
