...
#define BinaryElewise(inputA, inputB, output, op, stream) \
        ...
        inputA->dtype(), spec_t, "BinaryElewiseCuda", [&]() {                   \
          binary_elewise_broadcast_kernel<spec_t, spec_t>                       \
            <<<blocks, threads, 0, cuda_stream>>>(                              \
              inputA->data_ptr<spec_t>(), inputB->data_ptr<spec_t>(), size,     \
              op<spec_t, spec_t>(), output->data_ptr<spec_t>(), gpu_dimsA, gpu_dimsB,   \
              (size_t) inputA->ndim(), (size_t) inputB->ndim(),                 \
              gpu_output_strides, output_dim);                                  \
        });                                                                     \
    }                                                                           \
    else {                                                                      \
      HT_LOG_ERROR << "Inputs' datatype should be the same.";                   \
    }                                                                           \
...
__global__ void binary_elewise_kernel(const spec_a_t* inputA, const spec_b_t* inputB,
...
template <typename spec_a_t, typename spec_b_t, typename Operator>
__global__ void
binary_elewise_broadcast_kernel(const spec_a_t* inputA, const spec_b_t* inputB,
                                size_t size, Operator op, spec_a_t* output, uint* A_dims,
...
void AddElewiseCuda(const NDArray& inputA, const NDArray& inputB,
  ...
  BinaryElewise(inputA, inputB, output, kplus, stream);
  // HT_LOG_INFO << inputA << "\n" << inputB << "\n" << output; 
}

void SubElewiseCuda(const NDArray& inputA, const NDArray& inputB,
                    NDArray& output, const Stream& stream) {
  BinaryElewise(inputA, inputB, output, kminus, stream);
}

void MulElewiseCuda(const NDArray& inputA, const NDArray& inputB,
                    NDArray& output, const Stream& stream) {
  BinaryElewise(inputA, inputB, output, kmultiplies, stream);
}

void DivElewiseCuda(const NDArray& inputA, const NDArray& inputB,
                    NDArray& output, const Stream& stream) {
  BinaryElewise(inputA, inputB, output, kdivides, stream);
}
