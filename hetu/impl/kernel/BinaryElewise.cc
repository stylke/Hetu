...
void binary_elewise_cpu(const spec_t* inputA, const spec_t* inputB, size_t size,
...
template <typename spec_t, typename Operator>
void binary_elewise_broadcast_cpu(const spec_t* inputA, const spec_t* inputB,
...
void BinaryElewiseToolCpu(const NDArray& inputA, const NDArray& inputB,
  ...
  if (sizeA == sizeB) {
    ...
    HT_DISPATCH_INTEGER_AND_FLOATING_TYPES(
      inputA->dtype(), spec_t, "BinaryElewiseCpu", [&]() {
        binary_elewise_broadcast_cpu<spec_t>(
            inputA->data_ptr<spec_t>(), inputB->data_ptr<spec_t>(), size, op,
            output->data_ptr<spec_t>(), A_dims, B_dims,
            (size_t) inputA->ndim(), (size_t) inputB->ndim(),
            out_strides, output_dim);
      });
...
void AddElewiseCpu(const NDArray& inputA, const NDArray& inputB,
  ...
  HT_DISPATCH_INTEGER_AND_FLOATING_TYPES(
      inputA->dtype(), spec_t, "AddElewiseCpu", [&]() {
        ...
        BinaryElewiseToolCpu(inputA, inputB, output, op, stream);
      }); 
}
