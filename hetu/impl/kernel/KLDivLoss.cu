...
__global__ void kldivloss_kernel(const spec_t* pred, const spec_t* label,
  ...
  spec_t lglabel = hetu::cuda::cuda_log(label[label_offset]);
  // clip to -100 following PyTorch
  spec_t min_value = -100;
  loss[loss_offset] = label[label_offset] * (hetu::cuda::cuda_max(lglabel, min_value) - pred[pred_offset]); 
}
...
kldivloss_gradient_kernel(const spec_t* pred, const spec_t* label,
  ...
  output[out_offset] = - grad_loss[grad_loss_offset] * label[label_offset];
}
