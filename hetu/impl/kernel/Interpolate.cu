...
__device__ spec_t cubic_convolution1(const spec_t x, const spec_t a) {
    return ((a + 2.0) * x - (a + 3.0)) * x * x + 1.0;
}

template <typename spec_t>
__device__ spec_t cubic_convolution2(const spec_t x, const spec_t a) {
    return ((a * x - 5.0 * a) * x + 8.0 * a) * x - 4.0 * a;
}

template <typename spec_t>
__device__ spec_t kecubic_interp(spec_t x0, spec_t x1, spec_t x2, spec_t x3,
                                 spec_t t) {
    spec_t coeffs[4];
    spec_t A = -0.75;
    spec_t y1 = t;
...
template <typename spec_t>
__device__ void get_cubic_upsample_coefficients(spec_t coeffs[4], spec_t t) {
    spec_t A = -0.75;
    spec_t y1 = t;
    spec_t y2 = 1.0 - t;
    coeffs[0] = cubic_convolution2(spec_t(y1 + 1.0), A);
...
template <typename spec_t>
__global__ void interpolate_kernel(const spec_t *input, int64_t n, int64_t c,
  ...
  for (; idx < size; idx += stride) {
      ...
      for (int64_t k = 0; k < 4; k++) {
          int64_t tmp_h = max(min(in_h_id - 1 + k, in_h - 1), int64_t(0));
          int64_t tmp_w0 = max(min(in_w_id - 1, in_w - 1), int64_t(0));
          int64_t tmp_w1 = max(min(in_w_id + 0, in_w - 1), int64_t(0));
          int64_t tmp_w2 = max(min(in_w_id + 1, in_w - 1), int64_t(0));
          int64_t tmp_w3 = max(min(in_w_id + 2, in_w - 1), int64_t(0));
  ...
  for (; idx < size; idx += stride) {
    ...
    for (int64_t i = 0; i < 4; i++) {
      for (int64_t j = 0; j < 4; j++) {
        int64_t tmp_h = max(min(in_h_id - 1 + i, in_h - 1), int64_t(0));
        int64_t tmp_w = max(min(in_w_id - 1 + j, in_w - 1), int64_t(0));
...
void InterpolateCuda(const NDArray& input, NDArray& output,
  ...
  HT_DISPATCH_FLOATING_TYPES(
    input->dtype(), spec_t, "InterpolateCuda", [&]() {
      interpolate_kernel<spec_t><<<blocks, threads, 0, cuda_stream>>>(
...
void InterpolateGradientCuda(const NDArray& output, NDArray& input,
  ...
  HT_DISPATCH_FLOATING_TYPES(
    input->dtype(), spec_t, "ArraySetZeroCuda", [&]() {
  ...
  blocks.x = DIVUP(size, HT_DEFAULT_NUM_THREADS_PER_BLOCK);
  HT_DISPATCH_FLOATING_TYPES(
    input->dtype(), spec_t, "InterpolateGradientCuda", [&]() {
      interpolate_gradient_kernel<spec_t><<<blocks, threads, 0, cuda_stream>>>(
