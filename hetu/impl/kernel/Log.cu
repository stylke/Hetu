...
__global__ void log_kernel(const spec_t* input, size_t size, spec_t* output,
  ...
  output[out_offset] = hetu::cuda::cuda_log(input[in_offset]);
}
...
void LogCuda(const NDArray& input, NDArray& output, const Stream& stream) {
  ...
  HT_DISPATCH_FLOATING_TYPES(input->dtype(), spec_t, "LogCuda", [&]() {
