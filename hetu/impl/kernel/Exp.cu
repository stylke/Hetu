...
__global__ void exp_kernel(const spec_t* input, size_t size, spec_t* output,
  ...
  if (idx < size) {
    ...
    output[out_offset] = hetu::cuda::cuda_exp(input[in_offset]);
  }
}
...
void ExpCuda(const NDArray& input, NDArray& output, const Stream& stream) {
  ...
  HT_DISPATCH_FLOATING_TYPES(input->dtype(), spec_t, "ExpCuda", [&]() {
    exp_kernel<spec_t><<<blocks, threads, 0, cuda_stream>>>(
