...
__global__ void dropout2d_gradient_kernel(const spec_t* grad,
                                          ...
                                          float drop_rate, size_t size, size_t last_two,
  ...
  if (idx >= size)
    return;
  size_t leader = size_t(idx / last_two) * last_two;
  auto fw_offset = fw_out_offset_calculator->get(leader);
...
void Dropout2dGradientCuda(const NDArray& grad, const NDArray& fw_output,
  ...
  HT_DISPATCH_FLOATING_TYPES(grad->dtype(), spec_t, "Dropout2dGradientCuda", [&]() {
    dropout2d_gradient_kernel<spec_t><<<blocks, threads, 0, cuda_stream>>>(
      ...
      static_cast<float>(drop_rate), size, last_two_size,
      grad_offset_calculator, fw_out_offset_calculator,
      out_offset_calculator);
  });
