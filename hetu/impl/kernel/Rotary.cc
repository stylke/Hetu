...
void rotary_forward_cpu(const spec_t* input, const spec_t* cos,
                        ...
                        const int split_pattern, const int batch_size, const int seq_len,
...
#pragma unroll
        for(int s_id = 0; s_id < seq_len; ++s_id) {
            ...
            rotary_block_forward_cpu(input, cos, sin, out, s_id_for_freqs, offset_block, offset_block_dst, num_heads,
...
#pragma unroll
        for(int s_id = 0; s_id < seq_len; ++s_id) {
            ...
            rotary_block_backward_cpu(input, cos, sin, out, s_id_for_freqs, offset_block, offset_block_dst, num_heads,
                                head_dim, freqs_dim, stride_h, stride_d, o_stride_h, o_stride_d);
        }
    }
}
...
template <typename spec_t>
void rotary_varlen_forward_cpu(const spec_t* input, const spec_t* cos,
...
template <typename spec_t>
void rotary_varlen_backward_cpu(const spec_t* input, const spec_t* cos,
                                const spec_t* sin, spec_t* out, const int* cu_seqlens,
                                const int max_seqlen, const int* start_seq_ids,
                                const int* end_seq_ids, const int split_pattern, const int batch_size,
                                const int num_heads, const int head_dim, const int freqs_dim,
...
#pragma unroll
        for(int s_id = 0; s_id + start < end; ++s_id) {
            ...
            rotary_block_backward_cpu(input, cos, sin, out, s_id, offset_block, offset_block_dst, num_heads,
                                head_dim, freqs_dim, stride_h, stride_d, o_stride_h, o_stride_d);
        }
    }
}
...
void RotaryCpu(const NDArray& input, const NDArray& cos, const NDArray& sin, 
    ...
    HT_ASSERT(input->ndim() == 4) << "RotaryCpu requires input->ndim() == 4, got " << input->ndim();
    HT_ASSERT(out->ndim() == 4) << "RotaryCpu requires out->ndim() == 4, got " << out->ndim();
    ...
    input->dtype(), spec_t, "RotaryCpu", [&](){
        auto _future = cpu_stream.EnqueueTask(
        [input, cos, sin, out, start_seq_id, end_seq_id, split_pattern, b, s, h, d]() {
        rotary_forward_cpu<spec_t>(
            ...
            b, s, h, d, cos->shape(1), input->stride(0), input->stride(1),
...
void RotaryGradientCpu(const NDArray& input, const NDArray& cos, const NDArray& sin, 
                        NDArray& out, const int start_seq_id, const int end_seq_id,
                        const int split_pattern, const Stream& stream) {
    HT_ASSERT_CPU_DEVICE(input);
    HT_ASSERT_SAME_DEVICE(input, cos);
    ...
    HT_ASSERT(input->ndim() == 4) << "RotaryGradientCpu requires input->ndim() == 4, got " << input->ndim();
    HT_ASSERT(out->ndim() == 4) << "RotaryGradientCpu requires out->ndim() == 4, got " << out->ndim();
...
void RotaryVarlenCpu(const NDArray& input, const NDArray& cos, const NDArray& sin,
    ...
    HT_ASSERT(input->ndim() == 3) << "RotaryVarlenCpu requires input->ndim() == 3, got " << input->ndim();
    HT_ASSERT(out->ndim() == 3) << "RotaryVarlenCpu requires out->ndim() == 3, got " << out->ndim();
    ...
    HT_DISPATCH_FLOATING_TYPES(
    input->dtype(), spec_t, "RotaryVarlenCpu", [&](){
        auto _future = cpu_stream.EnqueueTask(
        [input, cos, sin, out, cu_seqlens, max_seqlen, 
            start_seq_ids, end_seq_ids, split_pattern, b, h, d]() {
        rotary_varlen_forward_cpu<spec_t>(
            input->data_ptr<spec_t>(), cos->data_ptr<spec_t>(), sin->data_ptr<spec_t>(),
            out->data_ptr<spec_t>(), cu_seqlens->data_ptr<int>(), max_seqlen, 
            start_seq_ids, end_seq_ids, split_pattern, b, h, d, cos->shape(1), 
...
void RotaryVarlenGradientCpu(const NDArray& input, const NDArray& cos, const NDArray& sin,
                             NDArray& out, const NDArray& cu_seqlens, const int max_seqlen,
                             const int* start_seq_ids, const int* end_seq_ids, const int split_pattern,
                             const Stream& stream) {
    // input为q或k, shape: {batch_size_mul_seq_len, num_heads, head_dim}, out相同
    ...
    HT_ASSERT(input->ndim() == 3) << "RotaryVarlenGradientCpu requires input->ndim() == 3, got " << input->ndim();
    HT_ASSERT(out->ndim() == 3) << "RotaryVarlenGradientCpu requires out->ndim() == 3, got " << out->ndim();
