...
__global__ void mul_elewise_kernel(const spec_t* inputA, const spec_t* inputB,
...
template <typename spec_t>
__global__ void
mul_elewise_broadcast_kernel(const spec_t* inputA, const spec_t* inputB,
  ...
  if (sizeA == sizeB) {
    ...
    HT_DISPATCH_INTEGER_AND_FLOATING_TYPES(
      inputA->dtype(), spec_t, "MulElewiseCuda", [&]() {
        mul_elewise_kernel<spec_t><<<blocks, threads, 0, cuda_stream>>>(
    ...
    HT_DISPATCH_INTEGER_AND_FLOATING_TYPES(
      inputA->dtype(), spec_t, "MulElewiseCuda", [&]() {
        mul_elewise_broadcast_kernel<spec_t>
