...
template <typename spec_t>
__global__ void
reduce_max_kernel(const spec_t* input, spec_t* output, int ndim_input,
...
template <typename spec_t>
__global__ void reduce_max_single_kernel(const spec_t* input, spec_t* output,
  ...
  if (ndim_reduce == 1) {
    ...
    HT_DISPATCH_INTEGER_AND_FLOATING_TYPES(
      in_arr->dtype(), spec_t, "ReduceMaxCuda", [&]() {
        reduce_max_single_kernel<spec_t><<<blocks, threads, 0, cuda_stream>>>(
    ...
    HT_DISPATCH_INTEGER_AND_FLOATING_TYPES(
      in_arr->dtype(), spec_t, "ReduceMaxCuda", [&]() {
        reduce_max_kernel<spec_t><<<blocks, threads, 0, cuda_stream>>>(
