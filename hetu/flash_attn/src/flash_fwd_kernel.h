...
#include "hetu/flash_attn/src/block_info.h"
#include "hetu/flash_attn/src/kernel_traits.h"
#include "hetu/flash_attn/src/utils.h"
#include "hetu/flash_attn/src/softmax.h"
#include "hetu/flash_attn/src/philox.cuh"
...
inline __device__ void compute_attn_1rowblock(const Params &params, const int bidb, const int bidh, const int m_block) {
    ...
    unsigned long long seed = params.philox_args.seed;
    unsigned long long offset = params.philox_args.offset + (bidb * params.h + bidh) * 32 + tidx % 32;
...
inline __device__ void compute_attn_1rowblock_splitkv(const Params &params, const int bidb, const int bidh, const int m_block, const int n_split_idx, const int num_n_splits) {
    ...
    int n_block_max = min(cute::ceil_div(binfo.actual_seqlen_k, kBlockN), (n_split_idx + 1) * n_blocks_per_split);
    if (Is_causal) {
        n_block_max = min(n_block_max,
                               cute::ceil_div((m_block + 1) * kBlockM + binfo.actual_seqlen_k - binfo.actual_seqlen_q, kBlockN));
    }
...
inline __device__ void combine_attn_seqk_parallel(const Params &params) {
    ...
    constexpr int kRowsPerLoadTranspose = kRowsPerLoadLSE > kMaxSplits ? kMaxSplits : kRowsPerLoadLSE;
