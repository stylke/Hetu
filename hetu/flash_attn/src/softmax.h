...
inline __device__ void apply_mask_causal(Tensor<Engine, Layout> &tensor, const int col_idx_offset_,
    ...
    for (int mi = 0; mi < size<0, 1>(tensor); ++mi) {
        ...
        for (int i = 0; i < size<0, 0>(tensor); ++i) {
            ...
            const int col_idx_limit = min(max_seqlen_k, row_idx + 1 + max_seqlen_k - max_seqlen_q);
            #pragma unroll
            for (int nj = 0; nj < size<1, 1>(tensor); ++nj) {
                const int col_idx_base = col_idx_offset + nj * 8;
                #pragma unroll
    ...
    const int col_idx_offset_, const int max_seqlen_k, const int row_idx_offset_)
{
    ...
    #pragma unroll
    for (int mi = 0; mi < size<0>(tensor); ++mi) {
        const int col_idx_limit = min(max_seqlen_k, 1 + row_idx_offset_ + get<0>(idx_rowcol(mi, 0)));
        #pragma unroll
