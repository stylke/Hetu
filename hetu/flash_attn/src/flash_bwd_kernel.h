...
inline __device__ void compute_dq_dk_dv_1colblock(const Params &params, const int bidb, const int bidh, const int n_block) {
    ...
    int m_block_min = !Is_causal ? 0 : max(0, (n_block * kBlockN + binfo.actual_seqlen_q - binfo.actual_seqlen_k) / kBlockM);
...
inline __device__ void compute_dq_dk_dv_1rowblock(const Params &params, const int bidb, const int bidh, const int m_block) {
    ...
    if (Is_causal) {
        n_block_max = min(n_block_max, cute::ceil_div((m_block + 1) * kBlockM, kBlockN));
    }
