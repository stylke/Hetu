          ...
          if (is_comm_op(grad->producer())) {
            ...
            if ((is_homo && src_ds_union.get(0).check_allreduce(dst_ds_union.get(0)))
              ...
              if (cur_dst_ds_union.is_empty()) {
                ...
                HT_ASSERT(cur_dst_ds_union.check_equal(dst_ds_union))
                  << "unreduced grad " << i << " is generated by all_reduce, dst_ds_union is " << dst_ds_union.ds_union_info()
                  << ", but is mismatched with cur_dst_ds_union " << cur_dst_ds_union.ds_union_info();
              }
            }
            if ((is_homo && src_ds_union.get(0).check_reducescatter(dst_ds_union.get(0)))
              ...
              if (cur_dst_ds_union.is_empty()) {
                ...
                HT_ASSERT(cur_dst_ds_union.check_equal(dst_ds_union))
                  << "unreduced grad " << i << " is generated by reduce_scatter, dst_ds_union is " << dst_ds_union.ds_union_info()
                  << ", but is mismatched with cur_dst_ds_union " << cur_dst_ds_union.ds_union_info();
              }
            }
          } else {
            ...
            if ((is_homo && src_ds_union.get(0).states(-2) > 1)
              ...
              if (cur_dst_ds_union.is_empty()) {
                ...
                HT_ASSERT(cur_dst_ds_union.check_equal(src_ds_union))
                  << "unreduced grad " << i << " ds_union is " << src_ds_union.ds_union_info()
                  << ", but is mismatched with cur_dst_ds_union " << cur_dst_ds_union.ds_union_info();
              }
            }
          }
    ...
    if (op->num_outputs() > 0) {
      ...
      for (auto& output : op->outputs()) {
        try {
          ...
          HT_RUNTIME_ERROR << "During reducing grads of tensor " << output << " with unreduced grads " << tensor_to_grads[output->id()]
