...
class Hetu_CNN(hetu.nn.Module):
    def __init__(self):
        ...
        to_type = hetu.bfloat16
        
        self.conv1.to(to_type)
        self.conv2.to(to_type)
        self.conv3.to(to_type)
        self.fc1.to(to_type)
        self.fc2.to(to_type)
...
class Hetu_ResidualBlock(hetu.nn.Module):
    def __init__(self, inchannel, outchannel, stride=1, torchblock=None):
        ...
        self.left = hetu.nn.Sequential(
            ...
            hetu.nn.BatchNorm(outchannel, eps=1e-5),
            hetu.nn.ReLU(),
        ...
        self.left.to(hetu.bfloat16)
        self.shortcut.to(hetu.bfloat16)
        if (torchblock != None):
...
class Hetu_ResNet(hetu.nn.Module):
    ...
    def __init__(self, ResidualBlock, num_classes=10, torchresnet=None):
        ...
        self.conv1.to(hetu.bfloat16)
        self.fc.to(hetu.bfloat16)
...
def hetu_train_ex(hetu_model, epoch, train_loader):
    with hetu.graph("define_and_run"):
        x = hetu.placeholder(hetu.bfloat16, shape=[-1, 3, 32, 32], name = "x")
        y = hetu.placeholder(hetu.bfloat16, shape=[-1, 10], name = "y")
    ...
    # loss = hetu.binary_cross_entropy(probs, y, "mean")
        optimizer = hetu.SGDOptimizer(lr=0.01, momentum = 0.0)
        scaler = hetu.GradScaler(256.0)
        train_op = scaler.minimize(optimizer, loss)
        update_op = scaler.update()
    ...
    for step, (x0, y0) in enumerate(train_loader):
        ...
        # testtime = executor.run([test_out], feed_dict={
        #     x: x1, 
        #     y: y1, 
        # })
        # testtime2 = executor.run([test_out2], feed_dict={
        #     x: x1, 
        #     y: y1, 
        # })
        # testtime3 = executor.run([test_out3], feed_dict={
        #     x: x1, 
        #     y: y1, 
        # })
        with hetu.graph("define_and_run"):
            pred_val, loss_val, _, _ = train_op.graph.run([pred, loss, train_op, update_op], feed_dict={x: x1, y: y1})
        ...
        if step % 200 == 0 :
            ...
            print('Epoch: {}, Step {}, Pred: {}, Loss: {}, Loss_val: {}'.format(epoch, step, pred_val, loss_val, loss_val.numpy(force=True)[0]))
    print("TIME:", time.time() - st)
    print(len(train_loader.dataset))

def hetu_test_ex(model, test_loader):
    with hetu.graph("define_and_run"):
        x = hetu.placeholder(hetu.bfloat16, shape=[-1, 3, 32, 32])
        y = hetu.placeholder(hetu.bfloat16, shape=[-1, 10])
    output = model(x)
    ...
    for x0, y0 in test_loader:
        ...
        with hetu.graph("define_and_run"):
            out = output.graph.run([output], feed_dict={
                x: x1, 
                y: y1, 
            })
