...
# Warning: Remember to set rtol = 1e-03, atol = 1e-03 in `test_utils.py`
...
class TestArithmeticOps(unittest.TestCase):
    ...
    def test_elementwise_add(self):
        for shape in TestArithmeticOps._test_elementwise_shapes:
            ...
            y_np = np.random.randn(*shape).astype(np.float16)
    ...
    def test_broadcast_div(self):
        for shape_x, shape_y in TestArithmeticOps._test_broadcast_shapes:
            x_np = np.random.randn(*shape_x).astype(np.float16)
            y_np = np.random.randn(*shape_y).astype(np.float16) + 10
            x = hetu.from_numpy(x_np)
            y = hetu.from_numpy(y_np)
            gt = x_np / y_np
            self.assertTrue(allclose(x / y, gt))
    ...
    def test_sqrt(self):
        for shape in TestArithmeticOps._test_elementwise_shapes:
            x_np = np.abs(np.random.randn(*shape)).astype(np.float16)
            x = hetu.from_numpy(x_np)
            gt = np.sqrt(x_np)
    ...
    def test_sum(self):
        for shape in TestArithmeticOps._test_elementwise_shapes:
            x_np = np.random.randn(*shape).astype(np.float16)
            y_np = np.random.randn(*shape).astype(np.float16)
            z_np = np.random.randn(*shape).astype(np.float16)
            x = hetu.from_numpy(x_np)
            y = hetu.from_numpy(y_np)
    ...
    def test_matmul_op(self):
        for shape_x, shape_y in TestMatMulOps._test_shapes:
            ...
            if GRAD_TEST:
              ...
              torch_optimizer.step()
              hetu_optimizer.minimize(hetu_loss)
              self.assertTrue(allclose(hetu_in, torch_in.to(torch.float32).cpu().detach().numpy()))
    
    # def test_linear_op(self):
    #     for shape_x, shape_y in TestMatMulOps._test_shapes:
    #         x_np = np.random.randn(*shape_x).astype(np.float16)
    #         w_np = np.random.randn(*shape_y).astype(np.float16)
    #         bias_np = np.random.randn(shape_y[-1]).astype(np.float16)
...
class TestBatchMatMulOps(unittest.TestCase):
    ...
    def test_batch_matmul_op(self):
        for shape_x, shape_y in TestBatchMatMulOps._test_shapes:
            x_np = np.random.randn(*shape_x).astype(np.float16)
            y_np = np.random.randn(*shape_y).astype(np.float16)
            gt = torch.bmm(torch.from_numpy(x_np).to("cuda:0"), 
                           torch.from_numpy(y_np).to("cuda:0")).cpu().numpy()
            x = hetu.from_numpy(x_np)
            y = hetu.from_numpy(y_np)
            ...
            if GRAD_TEST:
              torch_in = torch.tensor(x_np, requires_grad=True, device="cuda:0")
              torch_out = torch.bmm(torch_in, torch.from_numpy(y_np).to("cuda:0"))
              torch_loss = torch_out.sum()
              torch_optimizer = optim.SGD([torch_in], lr = 0.5)
              hetu_in = hetu.Tensor(x_np, requires_grad=True)
...
class TestActivationOps(unittest.TestCase):
    ...
    def test_relu_op(self):
        for shape in TestActivationOps._test_shapes:
            x_np = np.random.randn(*shape).astype(np.float16) - 0.5
            ...
            if GRAD_TEST:
              ...
              torch_loss = torch_out.sum()
              torch_optimizer = optim.SGD([torch_in], lr = 0.1)
              hetu_in = hetu.Tensor(x_np, requires_grad=True)
              hetu_out = hetu.relu(hetu_in)
              hetu_loss = hetu_out.sum()
              hetu_optimizer = hetu.SGDOptimizer([hetu_in], lr = 0.1)
              torch_loss.backward()
              torch_optimizer.step()
              hetu_optimizer.minimize(hetu_loss)
              self.assertTrue(allclose(hetu_in, torch_in.detach().numpy()))
    ...
    def test_conv2d_op(self):
        for shape in TestConv2dOps._data_shapes:
            ...
            for f_shape in TestConv2dOps._filter_shapes:
                f_np = np.random.randn(*f_shape).astype(np.float16)
                ...
                bias_np = np.random.randn(*bias_shape).astype(np.float16)
                bias = hetu.from_numpy(bias_np)
...
class TestNormOps(unittest.TestCase):
    ...
    def test_batchnorm_op(self):
        for shape in TestPoolOps._test_shapes:
            ...
            save_mean_np = np.empty(shape[1]).astype(np.float16)
            save_mean = hetu.from_numpy(save_mean_np)
            save_var_np = np.empty(shape[1]).astype(np.float16)
    ...
    def test_layernorm_op(self):
        for shape in TestPoolOps._test_shapes:
            for i in range(1, 4):
                ...
                x_np = np.random.randn(*shape).astype(np.float16)
                x = hetu.from_numpy(x_np)
                scale_np = np.ones(norm_shape).astype(np.float16)
                scale = hetu.from_numpy(scale_np)
                bias_np = np.zeros(norm_shape).astype(np.float16)
                bias = hetu.from_numpy(bias_np)
    ...
    def test_instancenorm_op(self):
        for shape in TestPoolOps._test_shapes:
            ...
            save_mean_np = np.empty(temp_shape).astype(np.float16)
            save_mean = hetu.from_numpy(save_mean_np)
            save_var_np = np.empty(temp_shape).astype(np.float16)
            save_var = hetu.from_numpy(save_var_np)
...
class TestLossOps(unittest.TestCase):
    ...
    def test_bce_op(self):
        ...
        for shape in TestLossOps._test_binary_label_shapes:
            probs_np = np.random.uniform(1e-10, 1, size=shape).astype(np.float16)
            labels_np = np.random.choice([0, 1], size=shape).astype(np.float16)
    ...
    def test_softmax_cross_entropy_op(self):
        ...
        for shape in TestLossOps._test_cross_entropy_label_shapes:
            probs_np = np.random.uniform(1e-10, 1, size=shape).astype(np.float16)
            #labels_np = np.random.uniform(0.25, 0.5, size=shape).astype(np.float16)
            labels_np = np.random.choice(range(16), size=(64,)).astype(np.int64)
            labels_onehot = torch.nn.functional.one_hot(torch.from_numpy(labels_np), 16).numpy().astype(np.float16)
...
#                 input_np = np.random.randn(*shape).astype(np.float16)
...
class TestOtherOps(unittest.TestCase):
    ...
    def test_interpolateop(self):
        ...
        for shape_x, shape_o in TestOtherOps._interpolate_test_shapes:
            x_np = np.random.randn(*shape_x).astype(np.float16)
            gt = torch.nn.functional.interpolate(torch.from_numpy(x_np).to("cuda:0"), shape_o, mode='bicubic').cpu().numpy()
            x = hetu.from_numpy(x_np)
    ...
    def test_whereop(self):
        ...
        for shape_x in TestOtherOps._onehot_test_shapes:
            ...
            x_np = np.random.randn(*shape_x).astype(np.float16)
            y_np = np.random.randn(*shape_x).astype(np.float16)
