...
#             # tensor + tensor
#             gt = x_np + y_np   #(torch.from_numpy(x_np).to(torch.bfloat16) + torch.from_numpy(y_np).to(torch.bfloat16)).numpy()
...
#             gt = x_np + c #(torch.from_numpy(x_np) + c).numpy()
...
#               hetu_in = hetu.Tensor(y_np, requires_grad=True, dtype=hetu.bfloat16)
...
class TestLossOps(unittest.TestCase):
    ...
    def test_softmax_cross_entropy_op(self):
        ...
        for shape in TestLossOps._test_cross_entropy_label_shapes:
            ...
            probs = hetu.from_numpy(probs_np).to(hetu.bfloat16)
            labels = hetu.from_numpy(labels_onehot).to(hetu.bfloat16)
            loss = hetu.softmax_cross_entropy(probs, labels)
            print(loss.numpy(force=True))
            self.assertTrue(allclose(loss, gt))

            if GRAD_TEST:
                torch_in = torch.tensor(probs_np, requires_grad=True)
                torch_out = torch.nn.functional.cross_entropy(torch_in.cuda(), torch.from_numpy(labels_np).cuda())
                torch_loss = torch_out.sum() * 256
                torch_optimizer = optim.SGD([torch_in], lr = 2000)
                hetu_in = hetu.Tensor(probs_np, requires_grad=True, dtype=hetu.bfloat16)
                hetu_out = hetu.softmax_cross_entropy(hetu_in, labels)
                hetu_loss = hetu_out.sum() * 256
                hetu_optimizer = hetu.SGDOptimizer([hetu_in], lr = 2000)
                torch_loss.backward()
                torch_optimizer.step()
                print(hetu_in.numpy(force=True))
                hetu_optimizer.minimize(hetu_loss)
                print(hetu_in.numpy(force=True))
                print(torch_in.detach().numpy())
                self.assertTrue(allclose(hetu_in, torch_in.detach().numpy()))
        print(sys._getframe().f_code.co_name)
