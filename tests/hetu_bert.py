...
class BertEmbeddings(object):
    ...
    def __call__(self, input_ids, token_type_ids):
        ...
        position_ids = ht.from_numpy(np.arange(seq_length).reshape(
            (1, -1)).repeat(batch_size, axis=0).astype(np.int64)).to_variable(False)
...
class BertSelfAttention(object):
    ...
    def __call__(self, hidden_states, attention_mask):
        ...
        attention_probs = ht.softmax(attention_scores)
...
class BertForPreTraining(object):
    ...
    def __init__(self, config):
        ...
        index_all = ht.from_numpy(np.arange(config.vocab_size).astype(np.int64)).to_variable(False)
...
class Embedding(object):
    def __init__(self, num_embeddings, embedding_dim, embedding_name=None, initializer=ht.nn.init.xavier_normal_):
        self.weight = ht.nn.Parameter(ht.empty([num_embeddings, embedding_dim], trainable=True))
...
class BertLayerNorm(object):
    def __init__(self, hidden_size, eps=1e-12):
        self.eps = eps
        self.scale = ht.nn.Parameter(ht.ones([hidden_size,]))
        self.bias = ht.nn.Parameter(ht.zeros([hidden_size,]))

    def __call__(self, input_tensor):
        return ht.layer_norm(input_tensor, self.scale, self.bias, self.eps)
...
class Dropout(object):
    ...
    def __call__(self, input_tensor):
        if self.dropout_prob is None or self.dropout_prob == 0.0:
            return input_tensor
        output = ht.dropout(input, 1.0 - self.p, False, False)
        return output
