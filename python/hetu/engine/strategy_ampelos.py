...
class TPGroup(Args):
    def __init__(
        ...
        self.unused = unused
        self.num_devices = tp_devices_num
    ...
    def __lt__(self, other):
        return self.num_devices < other.num_devices or (self.num_devices == other.num_devices and self.node_idx < other.node_idx)
    ...
    def split(self):
        tp_group1 = TPGroup(
            ...
            self.node_idx,
            self.tp,
            self.devices[:self.num_devices // 2],
            self.sr_list[:self.num_devices // 2],
            self.unused 
        )
        tp_group2 = TPGroup(
            self.ctxs,
        ...
        return tp_group1, tp_group2

    def __repr__(self):
        return f"tp_group:{self.devices} node_idx:{self.node_idx}, tp:{self.tp}, devices:{self.devices}, sr_list:{self.sr_list}"
...
class HMP:
    def __init__(self, groups):
        self.groups = []
        self.groups = groups
        self.num_devices = sum(self.groups)

    def __lt__(self, other):
        # 按照优先级从小到大排序
        return self.num_devices < other.num_devices
    
    def add_group(self, group):
        self.num_devices += group
        self.groups.append(group)

    def __repr__(self):
        return f"HMP(groups={self.groups}, num_devices={self.num_devices})"
...
class StrategyModel:
    ...
    def make_plans(self):
        ...
        clock = time.time()
        all_tp_groups, new_suspended_devices, \
        new_unused_devices, best_hetero_tp_list = self.solve_tp_arrangments_new()
        print(all_tp_groups)
        all_tp_groups = [all_tp_groups]
        # split TP Groups for some iters
        num_iter = 2
        for i in range(num_iter):
            last_tp_groups = all_tp_groups[-1]
            tmp = sorted(last_tp_groups, reverse=True)
            group_to_be_split = tmp[0]
            if (tmp[0].num_devices % 2 == 0):
                tmp1, tmp2 = tmp[0].split()
                tmp.remove(tmp[0])
                tmp.append(tmp1)
                tmp.append(tmp2)
                all_tp_groups.append(tmp)
            else:
                break
        ...
        if use_balance:
            ...
            objectives_list = []
            diff_tolerance = 2
            for tp_group in all_tp_groups:
                if ((len(tp_group) - 1) // self.dp + 1 > 8):
                    break
                pipeline_list, \
                l_value_list, \
                m_value_list, \
                delta_list, \
                objective_list = self.enumerate_balanced_pp_pattern(tp_group, len(new_suspended_devices))
                for idx in range(len(pipeline_list)):
                    if (delta_list[idx] <= diff_tolerance):
                        pipelines_list.append(pipeline_list[idx])
                        l_values_list.append(l_value_list[idx])
                        m_values_list.append(m_value_list[idx])
                        deltas_list.append(delta_list[idx])
                        objectives_list.append(objective_list[idx])
            
                # if len(pipelines_list) > 0:
                #     break        
            top_k = min(self.ctxs.top_k, len(objectives_list))
            top_k_idx = heapq.nsmallest(top_k, range(len(objectives_list)), key=lambda i: objectives_list[i])
            objectives_list = [objectives_list[i] for i in top_k_idx]
            l_values_list = [l_values_list[i] for i in top_k_idx]
            m_values_list = [m_values_list[i] for i in top_k_idx]
            pipelines_list = [pipelines_list[i] for i in top_k_idx]
        else:
            pipelines_list, \
            l_values_list, \
            m_values_list, \
            objectives_list = self.enumerate_pp_pattern(all_tp_groups[0], len(new_suspended_devices))

        print("solve pp arrangement time =", time.time() - clock)
        print(len(pipelines_list))
        total_strategy_num = len(pipelines_list)
        self.strategies = []
        self.ds_parallel_configs = []

        # self.tp = best_hetero_tp_list[0]
        ...
        for strategy_id in range(total_strategy_num):
            ...
            # 引入一些dummy devices占位
            cur_dp = len(pipelines)
            # print(f"cur_dp:{cur_dp}")
            # for pipeline_idx in range(self.dp):
            for pipeline_idx in range(cur_dp):
                for stage_idx in range(hetero_stages[pipeline_idx]):
                    ...
                    for rank_idx in range(start_rank_idx, start_rank_idx + self.tp):
                        ...
                        if rank_idx - start_rank_idx < tp_size:
                            ...
                            # print(f"suspended_rank_list:{suspended_rank_list}, "
                            #       f"new_suspended_devices:{new_suspended_devices}")
                            # print(f"unused_rank_list:{unused_rank_list}, "
                            #       f"new_unused_devices:{new_unused_devices}")
            ...
            print(f"ID:{strategy_id}, Strategy:{pipelines_list[strategy_id]}")
            print(f"score:{objectives_list[strategy_id]}")
            for dpr, ppl in enumerate(pipelines_list[strategy_id]):
                stage_list = []
                for sid, stage in enumerate(ppl):
                    stage_list.append(len(stage.devices))
                print(f"dp rank:{dpr}, stage_list:{stage_list}")
            # objectives:{objectives_list[strategy_id]},
            # print(f"Ds_parallel_config:{ds_parallel_config}")
    ...
    def solve_tp_arrangments_malleus(self):
    ...
    def solve_tp_arrangments_new(self):
        ...
        for node_idx in range(self.all_nodes_num):
            ...
            if node_unused_num == node_devices_num:
                continue
            node_available_devices_sr = {}
            for device_idx in node_devices_range:
                if (device_idx not in available_devices_sr):
                    continue
                node_available_devices_num += 1
                if available_devices_sr[device_idx] < 10:
                    node_active_device_num += 1
            # node_max_homo_devices_num = node_available_devices_num - self.tp
            node_max_homo_devices_num = 0
            assert node_available_devices_num > node_max_homo_devices_num, \
                f"{node_available_devices_num} <= {node_max_homo_devices_num}: unused ranks currently shouldn't be larger than tp - 1"
            node_available_devices_sr = {k: v for k, v in available_devices_sr.items() if k in node_devices_range}
            # 非straggler按照device的序号升序排列
            ...
            def find_best_tp_split():
                nonlocal best_hetero_tp_R_val, best_hetero_tp_list, best_node_available_devices_sr
                hetero_tp_max = self.tp
                # 寻找最优的hetero tp split方式
                begin_hetero_tp = hetero_tp_max
                start_idx = node_max_homo_devices_num - 1
                print(f"hetero_tp_max:{hetero_tp_max}")
                # while begin_hetero_tp >= 1:
                idx = start_idx
                hetero_tp = begin_hetero_tp
                hetero_tp_R_val = 0
                ...
                while hetero_tp >= 1:
                    idx = idx + hetero_tp
                    if idx > node_active_device_num - 1:
                        idx -= hetero_tp
                        hetero_tp = hetero_tp // 2
                        continue
                    hetero_tp_list.append(hetero_tp)
                    # print(f"start_idx:{start_idx}, idx:{idx}, hetero_tp_list:{hetero_tp_list}"
                    #         f"node_available_devices_sr:{node_available_devices_sr}")
                    relative_hetero_idx = int(math.log2(self.tp // hetero_tp))
                    alpha = self.ctxs.hetero_tp_alpha[relative_hetero_idx]
                    weight = self.ctxs.hetero_tp_weight[relative_hetero_idx]
                    sr = max(device_sr[1] for device_sr in node_available_devices_sr[idx - hetero_tp + 1: idx + 1])
                    hetero_tp_R_val += 1 / (alpha * sr * weight)
                    if idx == node_active_device_num - 1:
                        # remain_devices = node_available_devices_num - node_active_device_num
                        # if remain_devices > 0:
                        #     hetero_tp_list.append(remain_devices)
                        break
                if hetero_tp_R_val > best_hetero_tp_R_val:
                    best_hetero_tp_R_val = hetero_tp_R_val
                    best_hetero_tp_list = hetero_tp_list.copy()
            ...
            if straggler_num <= 1:
                ...
                for straggler_seq_len in range(1, straggler_num):
                    ...
                    for enum, index in enumerate(reversed(range(-1, len(node_available_devices_sr) - straggler_num))):
                        ...
                        '''
                        find_best_tp_split()
                        print(f"index {index} straggler_seq_len {straggler_seq_len} enum {enum}, node_available_devices_sr =", node_available_devices_sr, "and best_hetero_tp_list =", best_hetero_tp_list, "and best_hetero_tp_R_val =", best_hetero_tp_R_val)
                        if index != -1:
                            tmp = node_available_devices_sr.pop(index)
                            node_available_devices_sr.insert(index + straggler_seq_len, tmp)
                        break
            print("best_node_available_devices_sr =", best_node_available_devices_sr, "and best_hetero_tp_list =", best_hetero_tp_list)
            # final_used_devices = node_devices_num - self.tp + sum(best_hetero_tp_list)
            final_used_devices = sum(best_hetero_tp_list)
            print("final_used_devices =", final_used_devices, "best hetero tp list =", best_hetero_tp_list)
            # 常规tp组
            visited_devices = {}
            for tp_idx in range(node_max_homo_devices_num // self.tp):
            ...
            # 异构tp组
            start_hetero_idx = node_max_homo_devices_num
            # dead devices should be add into a hetero group for after generate
            remain_devices = node_available_devices_num - node_active_device_num
            # if remain_devices > 0:
            #     best_hetero_tp_list.append(remain_devices)
            ...
            for idx in range(final_used_devices, node_available_devices_num):
                new_suspended_devices.append(best_node_available_devices_sr[idx][0])
        print(f"new_suspended_devices:{new_suspended_devices}," 
              f"self.unused_devices:{self.unused_devices}")
        return all_tp_groups, new_suspended_devices, self.unused_devices, best_hetero_tp_list
    ...
    def enumerate_pp_pattern(self, all_tp_groups: List[TPGroup], num_dead_devices: int = 0):
        # 启发式
        # 大部分tp是正常的
        ...
        # support different dp
        num_alive_devices = self.all_devices_num - num_dead_devices
        
        hetero_stages_plans = []
        def get_stage_dict(num_pipelines, hetero_stages):
            stage_dict = {}
            stage_dict["num_pipelines"] = num_pipelines
            stage_dict["hetero_stages"] = hetero_stages
            return copy.deepcopy(stage_dict)
        
        if self.dp * self.pp == len(all_tp_groups) and num_dead_devices == 999999:
            hetero_stages = [self.pp for _ in range(self.dp)] 
            hetero_stages_plans.append(get_stage_dict(self.dp, hetero_stages))
        else:
            # heuristic
            # 目前策略是尽量让stage数平均
            dp_min = max((self.dp * num_alive_devices) // self.all_devices_num, 1)
            dp_max = self.dp
            dp_min1 = dp_max
            while (dp_min1 > dp_min):
                dp_min1 = dp_min1 // 2
            dp_min = dp_min1

            dp_min = dp_max
            #trick
            print(f"DP_MIN:{dp_min}, DP_MAX:{dp_max}, DP:{self.dp}, "
                  f"num_alive_devices:{num_alive_devices}, "
                  f"self.all_devices_num:{self.all_devices_num}")
            def is_pow2(num: int):
                while num > 1:
                    if num % 2 != 0:
                        return False
            ...
            for cur_dp in range(dp_min, dp_max + 1):
                if (not is_pow2(cur_dp)):
                    continue
                base_stages = len(all_tp_groups) // cur_dp
                remain_stages = len(all_tp_groups) - cur_dp * base_stages
                search_stage_gap = 1
                '''
                ...
                def search_hetero_stage_plan(pipeline_idx):
                    ...
                    if pipeline_idx == cur_dp:
                        if all_stages == len(all_tp_groups):
                            print(f"all_stages:{all_stages}, hetero_stages_plan{hetero_stages_plan},"
                                  f"len_tp_grp:{len(all_tp_groups)}")
                            if 0 not in hetero_stages_plan:
                                hetero_stages_plans.append(get_stage_dict(cur_dp, hetero_stages_plan))
                        return
                    for stage_num in range(base_stages - search_stage_gap, 
                                           base_stages + remain_stages + search_stage_gap + 1):
        ...
        def dfs(hetero_stages_plan_idx, pipeline_idx):
            ...
            if pipeline_idx == hetero_stages_plans[hetero_stages_plan_idx]["num_pipelines"]:
            ...
            def pipeline_dfs(
                ...
                if pipeline_idx != 0 and pipeline_straggler_tp_group_num >= 1 and pipeline_straggler_tp_group_num == pipelines_straggler_tp_group_num[pipeline_idx - 1]:
                    if pipeline[0].sr > pipelines[pipeline_idx - 1][0].sr:
                        return
                # 搜完一条pipeline接着搜下一条
                if stage_idx == hetero_stages_plans[hetero_stages_plan_idx]["hetero_stages"][pipeline_idx]:
        ...
        # for idx, pipelines_template in enumerate(all_pipelines_template):
        #     print(f"IDX:{idx}, NUM_PIPELINE:{len(pipelines_template)}\n"
        #           f"pipelines_template:{pipelines_template}\n"
        #           f"pipelines_template[0]:{pipelines_template[0]}\n"
        #           f"pipelines_template[0][0]:{pipelines_template[0][0]}")
        for pipelines_template in all_pipelines_template:
            ...
            print(y_values, hetero_values, is_unused, len(pipelines_template))
            is_solved, objective, \
            l_values, m_values = self.solve_pp_arrangement(y_values, 
                                                           hetero_values, 
                                                           is_unused,
                                                           cur_dp=len(pipelines_template))
            if not is_solved:
                continue
        ...
        for idx, obj in enumerate(objective_list):
            print(f"idx:{idx}, obj:{obj}")     
        ...
        # for idx, template in enumerate(top_k_pipelines_template):
        #     print(f"ppidx:{idx}, template:{template}")
        # exit(0)
        for top_idx, (pipelines_template, l_lists) in enumerate(zip(top_k_pipelines_template, top_k_l_values)):
            ...
            # 先尽量减少跨机grad reduce
            # 把能填的None填了
            # print(f"LEN:{len(pipelines_template)}, pipelines_template:{pipelines_template}," 
            #       f"l_lists:{l_lists}")
            cur_dp = len(pipelines_template)
            ...
            for stage_id in range(max_hetero_stages):
                ...
                for pipeline_id in range(cur_dp):
        ...
        # 最终返回top k个搜索出的策略
        return top_k_pipelines_template, top_k_l_values, top_k_m_values, top_k_objective
                                                            
    def enumerate_balanced_pp_pattern(self, all_tp_groups: List[TPGroup], num_dead_devices: int = 0):
        # 启发式
        # 大部分tp是正常的
        # straggler tp groups按straggler ratio从大到小排
        # Args.print_args_list(all_tp_groups)     
        ...
        # greedy search for balanced plan.
        balanced_pipelines = []
        group_sizes = [tp_group.num_devices for tp_group in all_tp_groups] 
        group_sizes = sorted(group_sizes, reverse=True)

        #key-device num, value-TP groups
        tp_group_dict = {}
        for tp_group in all_tp_groups:
            if tp_group.num_devices not in tp_group_dict.keys():
                tp_group_dict[tp_group.num_devices] = []
            tp_group_dict[tp_group.num_devices].append(tp_group)
        ...
        def partition_into_k_groups(nums, k):
            groups = [HMP([]) for _ in range(k)]
            for num in nums:
                hmp = heapq.heappop(groups)
                hmp.add_group(num)
                heapq.heappush(groups, hmp)

            group_sums = []
            for hmp in groups:
                group_sums.append(hmp.num_devices)
        ...
        for cur_dp in range(dp_min, dp_max + 1):
            if (not is_pow2(cur_dp)) or (len(group_sizes) < cur_dp):
                continue

            hmps, hmp_sums = partition_into_k_groups(group_sizes, cur_dp)
            order = 0
            for hmp in hmps:
                if order == 2:
                    hmp.groups = sorted(hmp.groups, reverse=True)
                else:
                    hmp.groups = sorted(hmp.groups)
                    if order == 1:
                        if (len(hmp.groups) > 2 and hmp.groups[0] < hmp.groups[-2]):
                            tmp = hmp.groups[0]
                            hmp.groups[0] = hmp.groups[-1]
                            hmp.groups[-1] = hmp.groups[-2]
                            hmp.groups[-2] = tmp

            delta = max(hmp_sums) - min(hmp_sums)

            delta_list.append(delta)

            print(f"cur_dp:{cur_dp}, hmps:{hmps} hmp_sums:{hmp_sums}, delta:{delta}")

            hmps_list.append(hmps)
            hmp_sums_list.append(hmp_sums)
        
        # ILP
        # assert len(hmps_list) > 0, "something wrong, no available strategy"
        objective_list = []
        l_values_list = []
        m_values_list = []
        legal_pipelines_template = []
        
        for hmps in hmps_list:
            print(hmps)
            pipelines_template = []
            for hmp in hmps:
                pipelines_template.append([])
                stages = hmp.groups
                for stage in stages:
                    pipelines_template[-1].append(tp_group_dict[stage][0].copy())
            print(pipelines_template)
            print_formatted_pipelines_template = [
                [TPGroup.print(item) for item in pipeline]
                    for pipeline in pipelines_template
            ]
        ...
        top_k_m_values = [m_values_list[i] for i in top_k_idx]
        top_k_hmps_list = [
            hmps_list[i] for i in top_k_idx
        ]
        top_k_deltas = [delta_list[i] for i in top_k_idx]
        
        # log
        for i in range(top_k):
            print("**********************************************") 
            print("hmps =", top_k_hmps_list[i])
            print("objective =", top_k_objective[i])
            print("l_values =", top_k_l_values[i])
            print("m_values =", top_k_m_values[i])
            print("delta =", top_k_deltas[i])  

        # place devices 
        top_k_pipelines_template = []
        for tidx, hmps in enumerate(top_k_hmps_list):
            cur_l_values = top_k_l_values[tidx]
            pipelines_template = []
            cp_tp_group_dict = copy.deepcopy(tp_group_dict)
            cur_layers = []
            sum_hetero_stages = 0
            acc_hetero_stages = []
            for hmp in hmps:
                cur_layers.append(0)
                acc_hetero_stages.append(0)
                pipelines_template.append([])
                sum_hetero_stages += len(hmp.groups)
            # print(sum_hetero_stages)
            while sum_hetero_stages > 0:
                min_cur_layer = 9999999
                min_dprank = -1
                for dprank, cur_layer in enumerate(cur_layers):
                    if (cur_layer < min_cur_layer):
                        min_dprank = dprank
                        min_cur_layer = cur_layer
                cur_hmp = hmps[min_dprank]
                cur_layers[min_dprank] += cur_l_values[min_dprank][acc_hetero_stages[min_dprank]]
                cur_stage_num = cur_hmp.groups[acc_hetero_stages[min_dprank]]
                pipelines_template[min_dprank].append(cp_tp_group_dict[cur_stage_num][0])
                cp_tp_group_dict[cur_stage_num].pop(0)
                acc_hetero_stages[min_dprank] += 1
                sum_hetero_stages -= 1
            
            stage_node_ids = []

            for hmp in hmps:
                stage_node_ids.append([])

            pipelines_templateA = copy.deepcopy(pipelines_template)
            for dprank, pipeline in enumerate(pipelines_templateA):
                for idx, tp_group in enumerate(pipeline):
                    for j in range(cur_l_values[dprank][idx]):
                        stage_node_ids[dprank].append(tp_group.node_idx)
            
            total_layers = cur_layers[-1]
            intra_dp_layers = 0
            for l in range(total_layers):
                node_id = stage_node_ids[0][l]
                flag = True
                for j in range(len(pipelines_templateA)):
                    if (stage_node_ids[j][l] != node_id):
                        flag = False
                        break
                if flag:
                    intra_dp_layers += 1
            intra_dp_layersA = intra_dp_layers
            ...
            while sum_hetero_stages > 0:
                ...
                for dprank, cur_layer in enumerate(cur_layers):
                    if (cur_layer < min_cur_layer):
                        ...
                        min_cur_layer = cur_layer
                cur_hmp = hmps[min_dprank]
                cur_layers[min_dprank] += cur_l_values[min_dprank][-1 - acc_hetero_stages[min_dprank]]
                cur_stage_num = cur_hmp.groups[-1 - acc_hetero_stages[min_dprank]]
                # pipelines_template[min_dprank].append(cp_tp_group_dict[cur_stage_num][0])
                pipelines_template[min_dprank].insert(0, cp_tp_group_dict[cur_stage_num][0])
                cp_tp_group_dict[cur_stage_num].pop(0)
                acc_hetero_stages[min_dprank] += 1
                sum_hetero_stages -= 1
            ...
            pipelines_templateB = copy.deepcopy(pipelines_template)
            for dprank, pipeline in enumerate(pipelines_templateB):
                for idx, tp_group in enumerate(pipeline):
                    for j in range(cur_l_values[dprank][idx]):
                        stage_node_ids[dprank].append(tp_group.node_idx)
            ...
            intra_dp_layersB = intra_dp_layers

            if intra_dp_layersA > intra_dp_layersB:
                pipelines_template = pipelines_templateA
            else:
                pipelines_template = pipelines_templateB

            # for hmp in hmps:
            #     pipelines_template.append([])
            #     stages = hmp.groups
            #     for stage in stages:
            #         pipelines_template[-1].append(cp_tp_group_dict[stage][0])
            #         cp_tp_group_dict[stage].pop(0)
            top_k_pipelines_template.append(pipelines_template)


        return top_k_pipelines_template, top_k_l_values, top_k_m_values, top_k_deltas, top_k_objective
    ...
    def solve_pp_arrangement(
        ...
        # B_div_b = dp * self.ctxs.normal_mbn
        # 最优解的值
        m_values = [] 
