...
class LayerNorm(Module):
    ...
    def __init__(self, normalized_shape: Union[int, List[int]], eps: float = 1e-5, device_groups = None) -> None:
        with hetu.graph("define_and_run"):
            ...
            self.eps = eps
            self.weight = hetu.ones(self.normalized_shape, requires_grad=True, device_groups=device_groups)    #hetu.nn.Parameter(hetu.ones(self.normalized_shape))
            self.bias = hetu.zeros(self.normalized_shape, requires_grad=True, device_groups=device_groups)    #hetu.nn.Parameter(hetu.zeros(self.normalized_shape))
