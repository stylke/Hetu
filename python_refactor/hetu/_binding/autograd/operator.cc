...
PyObject* PyPushOpCtx(PyObject*, PyObject* args, PyObject* kwargs) {
  ...
  static PyArgParser parser({
    "push_op_ctx(DeviceGroup device_group=None, int stream_index=None, DataType dtype=None, List[Tensor] extra_deps=None)"
  });
  auto parsed_args = parser.parse(args, kwargs);
  if (parsed_args.signature_index() == 0) {
    if (parsed_args.has(0))
      get_device_group_ctx().push(parsed_args.get_device_group(0));
    if (parsed_args.has(1))
      get_stream_index_ctx().push(parsed_args.get_stream_index(1));
    if (parsed_args.has(2))
      get_dtype_ctx().push(parsed_args.get_dtype(2));
    if (parsed_args.has(3))
      get_extra_deps_ctx().push(parsed_args.get_tensor_list(3));
    Py_RETURN_NONE;
  } else {
...
PyObject* PyPopOpCtx(PyObject*, PyObject* args, PyObject* kwargs) {
  HT_PY_FUNC_BEGIN
  static PyArgParser parser({
    "pop_op_ctx(bool pop_device_group=False, bool pop_stream_index=False, bool pop_dtype=False, bool pop_extra_deps=False)"
  });
  auto parsed_args = parser.parse(args, kwargs);
  if (parsed_args.signature_index() == 0) {
...
void AddOpContextManagingFunctionsToModule(py::module_& module) {
  auto sub_m = module.def_submodule("_internal_contxt");
  HT_RUNTIME_ERROR_IF(0 != PyModule_AddFunctions(sub_m.ptr(), PyCtx_methods))
